...
class Watch_sn_twitter(Watch):
    """
    this watch will check if you have new twitter messages
    """
    ...
    def __init__(self, specto, id, values):
        ...
        self.standard_open_command = "xdg-open %s" % self.url

        #Init the superclass and set some specto values
        Watch.__init__(self, specto, id, values, watch_values)

        self.cache_file = os.path.join(self.specto.CACHE_DIR, "twitter" + self.username + ".cache")
        self.twitter_id = self.read_cache_file()
    ...
    def check(self):
        """ Check for new messages on your twitter account. """
        try:
            twitter_api = Api(username=self.username, password=self.password)
            self.timeline = twitter_api.GetFriendsTimeline(since_id=self.twitter_id)
            self.num_notifications = len(self.timeline)
    ...
    def get_gui_info(self):
        return [(_("Name"), self.name),
                ...
                (_("New tweets"), self.num_notifications)]

    def get_balloon_text(self):
        """ create the text for the balloon """
        if self.num_notifications == 1:
            text = _("New tweet from <b>%s</b>") % (self.timeline[0].GetUser().name)
        else:
            i = 0 #show max 4 tweets
            author_info = ""
            while i < self.num_notifications and i < 4:
                author_info += self.timeline[i].GetUser().name + ", "
                if i == 3 and i < self.num_notifications - 1:
                    author_info += "and others..."
                i += 1
            author_info = author_info.rstrip(", ")
            text = _("New tweets from <b>%s</b>...\n\n... <b>totalling %d</b> new tweets.") % (author_info, len(self.timeline))
        return text
    ...
    def get_extra_information(self):
        ...
        text = ""
        while i < self.num_notifications and i < 4:
            name = self.timeline[i].GetUser().name
            subject = self.unescape(self.timeline[i].GetText())
    ...
    def read_cache_file(self):
        id = -1 #if first time, don't show new tweets
        id_ = ""
...
class Api(object):
  ...
  def GetFriendsTimeline(self, user=None, since_id=None):
